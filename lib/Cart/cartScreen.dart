...
class _CartScreenState extends State<CartScreen> {
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '1. Your Bill',
                          style: TextStyle(color: Colors.green, fontSize: 5),
                        ),
                      ],
                    ),
                  ),
  ...
  void onCheckOutPress() {
    ...
    if (discountEditingController.text != "0") {
      if (discountEditingController.text.contains("%")) {
        ...
        subTotal = subTotal - subTotal / 100 * int.parse(newValue);
      } else {
        subTotal = subTotal - int.parse(discountEditingController.text);
      }
    }
    ...
    if (texEditingController.text != "0") {
      if (texEditingController.text.contains("%")) {
        ...
        setState(() {
          newValue =
              texEditingController.text.replaceAll(new RegExp(r'[^\w\s]+'), '');
          print(newValue);
        });
        subTotal = subTotal - subTotal / 100 * int.parse(newValue);
      } else {
        subTotal = subTotal - int.parse(texEditingController.text);
      }
    }
  ...
  Future<OrderPlaceMsgModel?> PlaceOrder(var arg) async {
    print("Length:" + arg.length.toString());
    final String apiUrl =
        "https://new-apna-store.herokuapp.com/api/createorder";
    ...
    for (var items in arg) {
      ...
      print("cal" + (double.parse(items[5]) * items[2]).toString());
      totalPrice = totalPrice + (double.parse(items[5]) * items[2]);
    }
  ...
  postOrders(var items) async {
    final OrderPlaceMsgModel? user = await PlaceOrder(items);
